...
struct Stats {
    ...
    int   raw_bits;           // uncompressed frame size in bits
    int   estimated_bits;     // rough entropy-coded size in bits
    float compression_ratio;  // raw / estimated
...
int count_rle_symbols(const std::vector<std::vector<int16_t>>& zz_blocks)
{
    // each (run, level) pair is one RLE symbol
    // run = number of zeros before this nonzero value
    int symbols = 0;
    for (const auto& zz : zz_blocks) {
        for (int k = 0; k < 64; k++) {
            if (zz[k] != 0) symbols++;  // each nonzero = one symbol
            // zeros themselves aren't symbols — they're absorbed into
            // the run-length prefix of the next nonzero symbol
        }
    }
    return symbols;
}
                   ...
                   std::vector<std::vector<int16_t>>& zz_blocks)
{
    ...
    // each nonzero coefficient costs roughly 6 bits in CAVLC
    // (1-2 bits for run length + 4 bits for level magnitude + sign)
    // this is a rough model, real CAVLC varies by coefficient magnitude
    int rle_syms        = count_rle_symbols(zz_blocks);
    s.raw_bits          = H * W * 8;           // 1 byte per pixel, uncompressed
    s.estimated_bits    = rle_syms * 6;        // ~6 bits per nonzero symbol
    s.compression_ratio = static_cast<float>(s.raw_bits) / s.estimated_bits;
    s.bpp               = static_cast<float>(s.estimated_bits) / (H * W);
...
void run_demo(int quality)
{
    ...
    std::cout << "  Sparsity                 : " << stats.sparsity_pct << "%\n";
    // add these four lines:
    std::cout << "  Raw frame size           : " << stats.raw_bits / 8 << " bytes\n";
    std::cout << "  Estimated compressed     : " << stats.estimated_bits / 8 << " bytes\n";
